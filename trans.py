...
if __name__ == "__main__":
    ...
    for f in os.listdir(mypath):
        if f.endswith(".txt"):
            ...
            argparser.add_argument('to_language',help = 'Example of command to run: python TranslateText.py en fr')
            args = argparser.parse_args()
            extract_file = "{}.txt".format(os.path.splitext(f)[0])
            transformed_file = "t8.shakespeare_{}.txt".format(os.path.splitext(f)[0])
            translate_file = "t8.shakespeare_{}_{}_{}.txt".format(args.from_language, 
